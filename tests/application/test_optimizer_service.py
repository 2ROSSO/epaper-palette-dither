...
class TestGetParamDefs:
    ...
    def test_illuminant_has_7_params(self) -> None:
        service = OptimizerService()
        defs = service.get_param_defs(ColorMode.ILLUMINANT)
        assert len(defs) == 7  # 3 illuminant + 4 common
    ...
    def test_grayout_has_5_params(self) -> None:
        service = OptimizerService()
        defs = service.get_param_defs(ColorMode.GRAYOUT)
        assert len(defs) == 5  # 1 gamut + 4 common
        names = [d.name for d in defs]
        assert "gamut_strength" in names
        assert "illuminant_red" not in names

    def test_anti_saturation_has_4_params(self) -> None:
        service = OptimizerService()
        defs = service.get_param_defs(ColorMode.ANTI_SATURATION)
        assert len(defs) == 4  # 4 common

    def test_centroid_clip_has_4_params(self) -> None:
        service = OptimizerService()
