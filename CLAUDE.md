...
- **プラン履歴の保存**: プランを `.history/{ブランチ名}/` に保存する（セッション間のコンテキスト保持目的）
  - ファイル名: `plan-YYYYMMDD-HHMMSS.md`
  - 最大10件保持。超過時は古いファイルから削除する
  - `.history/` はローカル専用（`.gitignore` 対象）
...
## ドキュメント・プロジェクト管理
- **wip/ および pr/ ブランチでの実装完了ごとに README.md を最新の実装状態に更新する**
- pr/ブランチ完了時は CLAUDE.md も併せて更新する
...
│   ├── optimizer_service.py  # OptimizerService（Optuna TPE自動最適化、参照画像キャッシュ）
...
│   ├── image_metrics.py      # PSNR, SSIM, Lab ΔE, S-CIELAB ΔE(scipy高速化+参照キャッシュ), Histogram Correlation, composite score
